...
function checkTime() {
    fetch(
        ...
        .then(v => v.json())
        .then(r => {
            qSelAll<NodeListOf<HTMLDivElement>>(".full_screen_error").forEach(v => v.setAttribute("data-hidden", "true"))
            const diff = diffTime(new Date().getTime(), r["unixtime"] * 1000)
            qSel<HTMLDivElement>("#clock_not_correct_error").setAttribute("data-hidden", diff > timeDelayLimit ? "false" : "true")
            console.log(diff)
        })
}
